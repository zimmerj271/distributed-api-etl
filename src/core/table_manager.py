...
class TableManager:
    ...
    def create_table(
        ...
        Args:
            table_name: Fully qualified table name ('catalog.schema.table_name')
            schema: Spark StructType schema
            location: Optional external location
            partition_by: Optional partition columns
            table_properties: Optional table properties (e.g. delta.enableChangeDataFeed)
        """
        ...
        if table_exists and cfg.mode != "overwrite":
            ...
            self._logger.warning(f"Sink table OVERWRITE enabled. Table {cfg.name} will be replaced.")
        ...
        if cfg.table_properties:
            properties = ", ".join(f"'{k}' = '{v}'" for k, v in cfg.table_properties.items())
    ...
    def validate_schema(self, table_name: str, expected: StructType) -> None:
        ...
        for col_name in set(existing_fields.keys()) & set(expected_fields.keys()):
            ...
            if existing_field.nullable != expected_field.nullable:
                if existing_field.nullable and not expected_field.nullable:
                    ...
                    errors.append(
                        f"Cannot make nullable column '{col_name}' NOT NULL"
                    )
                else:
        ...
        if errors:
            ...
            raise ValueError(
                f"Schema incompatible for {table_name}:\n{error_msg}\n\n"
            ) 
